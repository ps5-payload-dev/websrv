...
const LOCALSTORE_NETWORK_LOCATIONS_KEY = "LOCALSTORE_NETWORK_LOCATIONS";
 ...
 * @returns {string[]}
 */
function getSavedNetworkLocations() {
    let temp_networkLocationsStr = localStorage.getItem(LOCALSTORE_NETWORK_LOCATIONS_KEY);
    if (!temp_networkLocationsStr) {
        return [];
    }

    return JSON.parse(temp_networkLocationsStr);
}

/**
 * @param {string} path
 */
function addToSavedNetworkLocations(path) {
    let networkLocations = getSavedNetworkLocations();
    networkLocations.push(path);
    localStorage.setItem(LOCALSTORE_NETWORK_LOCATIONS_KEY, JSON.stringify(networkLocations));
}

/**
 * @param {string} path
 */
function removeFromSavedNetworkLocations(path) {
    let networkLocations = getSavedNetworkLocations();
    networkLocations = networkLocations.filter(entry => entry !== path);
    localStorage.setItem(LOCALSTORE_NETWORK_LOCATIONS_KEY, JSON.stringify(networkLocations));
}